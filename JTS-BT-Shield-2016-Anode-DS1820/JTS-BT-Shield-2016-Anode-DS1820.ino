 ...
 * 
 * Weiterer Befehl Ax startet den Automatik-Modus, x ist die Wartezeit
 *  * je Farbänderung in Millisekunden
 *  
 * Zusaetzlich fragt diese Version einen Temperatursensor DS1820 an Pin
 * 6 ab und liefert den Wert als Tx, wobei x die Temperatur in Grad * 100 ist
 */
...
void pwmcount ( void )
{
  ...
  if ( pwm[IDX_R] >= pwm_count ) digitalWrite ( LED_R, LOW ); else digitalWrite ( LED_R, HIGH ); 
  if ( pwm[IDX_G] >= pwm_count ) digitalWrite ( LED_G, LOW ); else digitalWrite ( LED_G, HIGH ); 
  if ( pwm[IDX_B] >= pwm_count ) digitalWrite ( LED_B, LOW ); else digitalWrite ( LED_B, HIGH ); 
}
